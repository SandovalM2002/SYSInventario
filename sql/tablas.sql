...
AS BEGIN
	...
	IF NOT EXISTS (SELECT id_MRP FROM DETALLE_MRP WHERE id_MRP = @MRP) BEGIN
		IF (@PADRE = 0) BEGIN
			WHILE(@COUNT <= (SELECT MAX(periodo) FROM PLAN_MAESTRO where Nodo = @nodo))	BEGIN	
		...
		ELSE IF (@PADRE > 0) BEGIN
			...
			WHILE (@COUNT <= (SELECT COUNT(*) FROM DETALLE_MRP WHERE id_MRP = @ID_NODO_MRP)) BEGIN 
...
select * from DETALLE_MRP where id_MRP = 1
